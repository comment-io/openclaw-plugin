...
// Monitor
// ---------------------------------------------------------------------------
export async function monitorCommentDocsAccount(ctx) {
    ...
    const allowSet = new Set(allowFrom.map((h) => h.toLowerCase()));
    ...
    async function processNotification(ntf) {
        if (seenIds.has(ntf.id))
            return "duplicate";
        seenIds.add(ntf.id);
        trimSeenIds();
    ...
    while (!abortSignal.aborted) {
        ...
        if (!envelope || abortSignal.aborted)
            continue;
        if (envelope.timeout)
            continue;
...
function waitForNotification(profile, abortSignal, log) {
    return new Promise((resolve) => {
...
async function ackClaim(claimId, log) {
    await runClaimCommand("ack", claimId, log);
}
async function releaseClaim(claimId, log) {
    await runClaimCommand("release", claimId, log);
}
function runClaimCommand(op, claimId, log) {
